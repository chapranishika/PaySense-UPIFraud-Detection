...
**Tested, not deployed: was the TF-IDF ceiling a data problem or an architecture problem?** Fine-tuned `distilbert-base-uncased` on the *exact same* training data as the v3 deployment above — same rows, same eval sets, only the model type changed — to isolate the answer. It scored **83.0%** accuracy / **82.0%** correct-and-confident / **96.0%** confidence-gate pass rate, a larger jump than v3's own data-diversity improvement produced, on identical data. Real evidence the ceiling was architectural, not just a training-diversity shortfall. Deliberately not wired into production: the fine-tuned model is ~128x larger (268MB vs. v3's 2.1MB) and single-request inference measured 369ms vs. TF-IDF's sub-millisecond, and this environment has no way to verify the deployed backend's hosting tier has the memory headroom for `torch`+`transformers` alongside the existing XGBoost/LightLR/TF-IDF stack — an unverified OOM crash on the live service is worse than keeping the faster classifier. Full results in `CATEGORY_CLASSIFIER_V3_ATTEMPT.md` §3.6.
