...
# ── File paths ──────────────────────────────────────────────────────────────
ZIP_SPAM   = "/mnt/user-data/uploads/Spam_sms.zip"
ZIP_FRAUD  = "/mnt/user-data/uploads/Financial_Fraud_Dataset.zip"

PATH_TXN   = "UPI Payment Transactions India/transactions.csv"
PATH_USR   = "UPI Payment Transactions India/users.csv"
PATH_MRC   = "UPI Payment Transactions India/merchants.csv"
PATH_SYNTH = "Financial Fraud Dataset/synthetic_fraud_dataset.csv"

OUTPUT_CSV = "/mnt/user-data/outputs/paysense_master_dataset.csv"
...
with zipfile.ZipFile(ZIP_SPAM) as z:
    df_txn  = pd.read_csv(z.open(PATH_TXN))
    df_usr  = pd.read_csv(z.open(PATH_USR))
    df_mrc  = pd.read_csv(z.open(PATH_MRC))

with zipfile.ZipFile(ZIP_FRAUD) as z:
    df_syn  = pd.read_csv(z.open(PATH_SYNTH))
...
#  The anchor stores day_of_week as string ("Monday", "Tuesday" …).
#  Supplement rows got the anchor mode string. Convert to ordinal int
...
# ── 8.4  user_city_tier → ordinal int (Tier 1 > Tier 2 > Tier 3 by urbanity) ─
