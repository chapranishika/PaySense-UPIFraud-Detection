...
A stratified random split (80/20, `random_state=42`), verified by reading
`paysense_phase3.py` and `resweep_threshold_against_ensemble.py` directly
— both use identical split parameters and dropped columns, confirmed by
checking they reproduce the same 253-fraud held-out partition. SMOTE is
applied after the split, training partition only. **The real gap this
split has, found in the same audit:** it doesn't stratify or otherwise
account for `data_source` — so the ~35% of rows from a source with a
near-tautological label relationship land in train and test in the same
proportion, and the test set can't detect the resulting inflation because
it shares the contamination. That's a genuinely more important finding
than "was it stratified" — see the ROC-AUC/PR-AUC-by-source breakdown
below.
...
The split mechanism itself is fine — stratified random, SMOTE correctly
applied after the split, preprocessor fit train-only, verified by reading
the code directly. What's *not* fine, and I'd say so before being asked:
the split doesn't account for `data_source`, so a ~35% subset with a
near-tautological label relationship is proportionally present in both
train and test — meaning the test set shares the same contamination it
should be catching. "Valid" in the mechanical sense (no row appears in
both, stratified correctly) is different from "valid" in the sense that
matters here (does the test set actually measure generalization to
organic fraud) — and by that second standard, it's compromised. I'd lead
with that distinction, not wait for the interviewer to find it.

**Q: What evidence says your model is better than the baseline?**
On the full (contaminated) test set: PR-AUC 0.5498 against a random
baseline of ~0.0421 — 13.05× above baseline. But that number is inflated:
on the organic-only subset, PR-AUC is 0.1138 against that subset's own
~4.01% base rate — about 2.8× baseline. Still real, positive evidence the
model beats chance on organic data, just a much smaller margin than the
headline claim.

**Q: Where can leakage occur?**
Precisely characterized now, not just flagged: it's not classic
train/test leakage (no row appears in both splits, no future information,
no target-derived feature) — it's a label-validity problem. A third of
the training data's fraud labels come from an external dataset whose own
labels were generated by thresholding two features the model also sees.
The model can solve that third by memorizing the threshold rather than
learning fraud patterns, and because the test set inherits the same
mixture, it can't detect this: source-stratified evaluation shows the
model gets 100% recall on that subset and 2.55% on the organic remainder.
...
Not the threshold itself — I fully traced that: a full 0.05–0.95 sweep
against the real ensemble confirms *no* threshold satisfies both
Recall≥75% and Precision≥50% simultaneously, so 0.50 (max-F1 fallback) is
genuinely the best available operating point, not an oversight. What I'm
actually least confident about is the fact that finding surfaced: on the
organic (non-tautological) slice of the same test set, recall at 0.50 is
2.55% (4/157), not 39.53% — meaning the real, deployable fraud-catching
ability of this model is far weaker than any number in this project has
ever reported. I'm confident the finding itself is correct (the TP/FN
counts reconcile exactly against the official confusion matrix), but I'm
not confident about the right next step — dropping the contaminated
supplement source outright would shrink the training set by a third, and
nobody has tested what that does to the model.
