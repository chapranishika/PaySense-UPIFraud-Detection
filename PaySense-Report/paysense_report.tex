...
Paytm notifications), accuracy drops to \textbf{72.5\%}, and only 62.0\% of
predictions both classify correctly and clear the Android client's own
0.65 confidence gate. The mechanism was traced directly: the fitted TF-IDF
vocabulary (821 tokens) is entirely mined from the 40 training templates,
so novel sentences sharing no content word with training collapse to an
identical, label-independent default prediction (30.0\% accuracy, near the
20\% random baseline for 5 classes), while novel sentences that happen to
reuse one training token score near training-level (86.7\%) but often via
single-keyword lookup rather than narration understanding---see
\texttt{CATEGORY\_CLASSIFIER\_GENERALIZATION.md} for the full breakdown,
