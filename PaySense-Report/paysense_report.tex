...
achieves a ROC-AUC of \textbf{0.8863} and a PR-AUC of \textbf{0.5339}---12.7
times above the random-classifier baseline---with 98.98\% precision at the
deployed operating threshold ($\tau=0.40$). An honest threshold sweep
identifies a recall ceiling of 69.96\%; Platt Scaling was subsequently
...
unchanged---the ceiling is a ranking limitation of the model on its current
feature set, not a probability-scale artifact, and is reported honestly as
an open limitation rather than a solved problem. The complete system is
...
\begin{table}[H]
\centering
\caption{Model Performance at Deployed Threshold $\tau^* = 0.40$}
\label{tab:results}
\begin{tabular}{@{}lr@{}}
\toprule
Metric & Value \\
\midrule
ROC-AUC                  & 0.8863 \\
PR-AUC (primary)         & 0.5339 \\
Precision (Fraud class)  & 98.98\% \\
Recall (Fraud class)     & 38.34\% \\
F1-Score (Fraud class)   & 0.5527 \\
True Positives           & 97 \\
False Positives          & 1 \\
False Negatives          & 156 \\
True Negatives           & 5{,}746 \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note: metrics above were recomputed 2026-08-22 directly against the
artifacts on disk (\texttt{paysense\_model.pkl}, \texttt{paysense\_}
\texttt{preprocessor.pkl}, \texttt{paysense\_master\_dataset.csv}). An
earlier draft of this table (ROC-AUC 0.8851, 66.14\%/52.17\% at $\tau=0.40$,
161 fraud rows in the test set) had drifted from what those artifacts
actually produce, most likely dating to a run of \texttt{paysense\_phase3.py}
before the artifacts were last retrained or the master dataset last
regenerated -- the mismatch went unnoticed because the stale numbers were
internally consistent with each other, not because anything was verified
against the frozen model directly.}
...
The PR-AUC of 0.5339 should be contextualised against the baseline of a
...
model achieves a PR-AUC that is \textbf{12.7 times} above this random
...
\midrule
0.05 & 17.15\% & 69.96\% & 0.275 & 177 / 855 \\
0.10 & 30.36\% & 49.80\% & 0.377 & 126 / 289 \\
0.15 & 51.34\% & 45.45\% & 0.482 & 115 / 109 \\
0.20 & 68.15\% & 42.29\% & 0.522 & 107 / 50  \\
0.25 & 83.47\% & 39.92\% & 0.540 & 101 / 20  \\
0.30 & 89.09\% & 38.74\% & 0.540 & 98  / 12  \\
0.35 & 96.04\% & 38.34\% & 0.548 & 97  / 4   \\
0.40 & \textbf{98.98\%} & \textbf{38.34\%} & \textbf{0.553} & 97  / 1   \\
0.45 & 100.0\% & 37.94\% & 0.550 & 96  / 0   \\
0.50 & 100.0\% & 37.94\% & 0.550 & 96  / 0   \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note: recomputed 2026-08-22 directly against the on-disk artifacts
(test set: 6{,}000 rows, 253 fraud). $\tau=0.40$ is the frozen, deployed
threshold -- it is the max-F1 point among all ten swept values, since none
meet the business constraint of Recall $\geq 75\%$, so selection falls back
to unconditional max-F1, exactly as the frozen \texttt{paysense\_threshold.pkl}
(0.40) reflects.}

\textbf{Honest finding:} Even at the most aggressive threshold
($\tau = 0.05$), the model's recall ceiling is \textbf{69.96\%}. Of the
253 fraud cases in the test set, 76 produce a fraud probability below
0.05---effectively invisible to the model regardless of threshold
selection. These are likely edge-case fraud patterns, possibly those
generated by SMOTE interpolation near the class boundary, that the model
ranks no differently from ordinary legitimate transactions. This was
originally framed here as a \textit{probability calibration} problem; that
framing was tested directly and found to be wrong (see below).

\textbf{Tested and rejected --- Platt Scaling does not move this ceiling.}
Platt Scaling~\cite{platt1999} fits a logistic function to the model's raw
output probabilities using a held-out calibration set:
...
equivalent point, the same 76 of 253 fraud rows remain below threshold,
unchanged. \textbf{The 69.96\% ceiling is therefore a ranking/discrimination
limitation of the model on its current feature set, not a probability-scale
artifact, and no recalibration method can fix it.} What Platt scaling can
legitimately affect is probability \textit{reliability} (whether "0.7"
means "70\% of the time this is fraud"), which is a different question,
addressed next---and even there, the result did not favour Platt scaling:
...
Calibration Error (a noisier metric at this sample size). Closing the
actual recall ceiling would require better features that separate the 76
low-ranked fraud rows from legitimate transactions, a different base model,
or accepting the ceiling as a property of this feature set pending further
data.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        curve plateaus at 69.96\% at $\tau = 0.05$. Tested directly
        (\texttt{PLATT\_SCALING\_RESULT.md}): this is a ranking ceiling, not
        a probability-scale one---Platt scaling leaves it unchanged.
        ...
        ($\tau^*=0.40$) metric comparison. The gap the threshold sweep buys
        here is small (F1 0.5527 vs. 0.5501, one additional true positive
...
\textbf{Recall ceiling (ranking limitation, not calibration).} The 69.96\%
recall ceiling identified in Section~V-D remains the highest-priority
limitation. It was originally attributed to probability miscalibration and
Platt Scaling was proposed as the fix; both were tested
(Section~V-D, \texttt{PLATT\_SCALING\_RESULT.md}) and the recall ceiling was
found to be completely unaffected by Platt Scaling, exactly as
monotonic-transform theory predicts. The ceiling is therefore a genuine
ranking/discrimination limit of the current 40-feature vector and XGBoost
configuration: 76 of 253 test-set fraud rows are ranked, by the model
itself, no differently from ordinary legitimate transactions, and no
post-hoc recalibration of the probability axis can change that ranking.
Closing it requires either additional or different features that separate
these specific fraud rows from legitimate ones, a different base model or
ensemble member, or accepting it as a documented property of this dataset
and feature set pending further data collection.
...
over a FastAPI backend. The system achieves a ROC-AUC of 0.8863 and
PR-AUC of 0.5339 on held-out synthetic test data, with 98.98\% precision at
the deployed operating threshold ($\tau=0.40$).
...
69.96\%. Platt Scaling was proposed, then implemented and tested against it:
the ceiling is unchanged by calibration, as a strictly monotonic
probability transform cannot alter a classifier's ranking of its inputs.
The ceiling is therefore correctly understood as a ranking limitation of
the current feature set and model, an open limitation rather than a solved
problem, requiring better features or a different model to close.
